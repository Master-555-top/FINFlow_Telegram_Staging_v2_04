...
- `npm ci --ignore-scripts`: passed earlier in v1.82 flow.
- Initial `npm run lint`: found missing `dailyDecisionChatContext` memo after wiring the chat call.
- Fix applied: added `dailyDecisionChatContext` memo in `DailyQuickInputPanel.tsx`.
