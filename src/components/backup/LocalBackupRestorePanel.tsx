  ...
  function applyRestore() {
    ...
    if (!result.ok) {
      setMessage(result.rolledBack
        ? `Restore не выполнен: ${result.reason}. Автоматический откат подтверждён.`
        : `Restore не выполнен: ${result.reason}. Автоматический откат тоже не завершён — не закрывайте страницу и сохраните текущие данные.`);
      setBusy(false);
      return;
    }
