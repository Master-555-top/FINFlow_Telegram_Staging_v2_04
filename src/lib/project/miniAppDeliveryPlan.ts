export const MINI_APP_DELIVERY_PLAN_VERSION = 'mini_app_delivery_plan_v2_49' as const;
...
const areas: MiniAppDeliveryArea[] = [
  {
    ...
    status: 'usable_local',
    weight: 10,
    summary: 'Основная оболочка и навигация уже есть; v2.49 добавил Final Local MVP Smoke и закрепил visual baseline lock без глобального редизайна.',
    done: ['7 главных вкладок', 'Telegram viewport hook', 'текущий dark/glass визуал', 'Sleep 3-tab MVP', 'Real Daily Use card v2.44', 'Daily Save QA v2.45', 'Telegram/Supabase Preflight v2.46', '7-day Real Data Week Test v2.47', 'Real Usage Gaps pass v2.48', 'Final Local MVP Smoke v2.49', 'visual baseline lock v2.49'],
    remaining: ['телефонный screenshot bug log', 'нижнее меню/safe-area pass на iOS/Android', 'точечный UI-polish по скриншотам']
  },
  {
    ...
    done: ['section-scoped History Engine', 'storage/reset/export', 'legacy key registry', 'canonical section map v2.32', 'money/work write adapter preview v2.33', 'Money Engine consumer v2.34', 'template apply preview v2.38', 'work shift lifecycle v2.38', 'Local Apply Center v2.42', 'Real Daily Use hardening v2.44', 'Period History + Daily Save QA v2.45', 'device/preflight gates v2.46', 'Final Local MVP smoke chain v2.49'],
  ...
  },
  {
    ...
    done: ['automation scope defined', 'security rules defined', 'webhook contract v2.41', 'daily report dry-run v2.41', 'backup/import/cloud workflow drafts', 'credentials policy', 'dry-run API without secrets', 'external n8n off check in staging gaps v2.48'],
  ...
  },
  {
    ...
    done: ['lint/build/audit scripts', 'Telegram checks panel', 'deployment checklists', 'cloud queue/conflict smoke path v2.40', 'n8n dry-run API smoke path v2.41', 'Local Apply confirm/rollback smoke path v2.42', 'CSV/JSON mapper smoke path v2.43', 'Real Daily Use hardening smoke surface v2.44', 'Daily Save QA v2.45', 'security regression tests from audited Codex sync', 'Telegram/Supabase preflight runbook v2.46', 'Real Data Week Test panel v2.47', 'Real Usage Gaps QA panel v2.48', 'Final Local MVP Smoke panel v2.49', '2 final-smoke regression tests v2.49'],
...
export function buildMiniAppDeliveryPlan(): MiniAppDeliveryPlan {
  ...
  return {
    ...
    realisticBuildsLeft: 'примерно 1 финальный phone/staging pass до сильного local-first MVP: final smoke есть, осталось прогнать на телефоне и закрыть bug log по скриншотам',
    nextBuild: 'v2.50 — Phone Screenshot Bugfix + Release Candidate: пройти Telegram phone smoke, закрыть реальные баги UI/данных и подготовить local-first RC без cloud writes',
    areas,
    criticalPath: [
      'прогнать Final Local MVP Smoke на телефоне и закрыть blockers',
      ...
      'полировать UI только по скриншотам, не ломая locked baseline'
    ],
    nonGoalsNow: [
